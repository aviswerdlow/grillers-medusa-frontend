...
test("unavailable status does not silently revoke the unchanged phone preference",async()=>{const f=form();f.set("sms_marketing_choice_unavailable","true");await updatePrimaryContact(null,f);expect(updateCustomer).toHaveBeenCalledWith({phone:"4045550101",metadata:{}})})
