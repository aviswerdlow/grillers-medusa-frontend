...
const inputLabel = (label: string) => new RegExp(`^${label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*\\*?$`)
...
it("names login fields and associates the returned error with keyboard-focused inputs", async () => {
  ...
  const password = screen.getByLabelText(inputLabel("Password"), { selector: "input" })
...
it("names registration and reset-request controls without sending or granting consent", () => {
  ...
  for (const label of [
    ...
    expect(screen.getByLabelText(inputLabel(label), { selector: "input" })).toHaveAccessibleName(label)
  }
...
it.each([
  ...
  async (password, confirm, field, error) => {
    ...
    await user.type(screen.getByLabelText(inputLabel("New password"), { selector: "input" }), password)
    await user.type(screen.getByLabelText(inputLabel("Confirm new password"), { selector: "input" }), confirm)
    await user.click(screen.getByRole("button", { name: "Update password" }))
    const invalid = screen.getByLabelText(inputLabel(field), { selector: "input" })
...
it("keeps two address forms independently named and focused", async () => {
  ...
  for (const label of [
    ...
    const first = shipping.getByLabelText(inputLabel(label), { selector: "input" }),
      second = billing.getByLabelText(inputLabel(label), { selector: "input" })
