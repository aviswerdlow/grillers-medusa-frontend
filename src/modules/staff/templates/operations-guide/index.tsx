...
const publicSurfaces = [
  ...
  {
    route: "/us/page/order-sms-terms",
    name: "Order text terms",
    staffUse:
      "Use when a customer asks what the optional UPS shipping and tracking text program covers, including message limits and STOP or HELP instructions.",
  },
  {
    route: "/us/page/order-sms-privacy",
    name: "Order text privacy",
    staffUse:
      "Use when a customer asks how the phone number and order-text consent are used for UPS shipping and tracking updates.",
  },
  {
    route: "/us/recipes",
    name: "Recipe hub",
...
const sections: GuideSection[] = [
  ...
  },
  {
    ...
    watch: [
      ...
      "If a Hechsher badge is wrong, correct the item-level source mapping; do not infer certification from a product title or description.",
  ...
  },
  {
    ...
    useFor: [
      ...
      "Tennessee route pickup and Dallas consolidated shipping orders.",
      "UPS shipping orders.",
    ...
    howTo: [
      ...
      "A Tennessee shipping address unlocks every active Tennessee route stop, including Memphis and Nashville. A Texas shipping address unlocks Dallas Consolidated Shipping. The customer still chooses the exact stop and one of its published dates.",
      "If the customer changes the shipping address, checkout clears address-dependent delivery or UPS selections and asks for a valid method again. Plant pickup can remain selected because it is not tied to the customer's ZIP.",
      ...
      "For UPS shipping, the payment step shows a separate unchecked shipping and tracking text box. It is optional, covers one enrollment confirmation plus later UPS shipping or tracking updates up to six total messages for that order, and is separate from marketing-text consent.",
      "Once the reviewed checkout is activated, Review your order appears in the payment step. Compare the address, arrival or pickup date, receipt email, quantities, fixed or per-pound pricing, shipping and estimate. Use its edit links before placing the order; changed or expired details require a fresh review.",
      "Before activating reviewed checkout, confirm each invoice account has its own approved payment terms. The activated review holds missing terms for an office correction. The existing checkout retains its current account behavior during the transition.",
      "Staff phone collection at placement is available only for fixed-price items in this candidate. For food priced by packed weight, use the saved-card customer checkout or send a checkout link, so the final amount is charged after packing.",
      "If a reviewed order placement is uncertain, use Check order status for the same cart. If that control is unavailable, ask the office to confirm the existing order before trying again. For a confirmed phone payment with failed order confirmation, use Retry order confirmation. Do not collect a second payment.",
    ...
    watch: [
      "These reviewed-checkout changes are a launch candidate and still need the integrated native/provider rehearsal before activation. They do not enable post-order edits. Peter's approved correction rules and the accounting/inventory amendment workflow remain launch gates.",
      "During the transition, ordinary checkout stays available and explains that per-pound prices are estimates and the final food total uses the packed weight. Once reviewed checkout is activated, native Admin draft conversion is disabled because it lacks that review. Create orders through customer or staff checkout.",
      "The original reviewed price, contact and item identity are retained. Editing a profile or catalog item cannot rewrite an existing order's acceptance. Use the approved order-exception procedure for corrections.",
  ...
  },
  {
    id: "b2b-invoice",
    eyebrow: "B2B invoice",
    title: "Invoice terms for business accounts",
    summary:
      "Business accounts may apply for invoice terms. Invoice checkout is currently off, so even approved accounts use card payment for current orders. When invoice checkout is enabled, eligible approved accounts may pay on their terms and the unpaid invoice ages in QuickBooks accounts receivable.",
    useFor: [
      "A synagogue, school, caterer, or other business requesting invoice terms for eligible future orders.",
      "Reviewing and approving, or declining, a business that applied for invoice terms.",
      "Explaining why an approved account still sees only card payment while invoice checkout is off.",
    ],
    howTo: [
      ...
      "Confirm the approved account terms and credit limit against Peter's QuickBooks customer record. A Medusa approval badge alone does not make invoice payment available at checkout; invoice checkout has a separate release gate.",
      "While invoice checkout is off, approved accounts complete the standard card checkout like other customers. Once enabled, an eligible approved account can choose Pay by invoice; that choice places the order without a card or final-charge card hold.",
      "An invoice order placed after enablement still goes through Pack and Finalize for picking and weighing. The final weighed total is invoiced, not charged to a card.",
    ...
    watch: [
      "While invoice checkout is off, no customer can choose Pay by invoice. Once enabled, the option remains limited to eligible approved accounts; the backend rejects an invoice attempt without authorization.",
      "Do not require a backup card as a condition of invoice approval. A card used for a current order while invoice checkout is off is payment for that order, not a standing backup card.",
      ...
      "An invoice order placed after enablement remains in QuickBooks accounts receivable until payment by Zelle, check, or wire is recorded on the account's terms. Collection is manual in QuickBooks for now.",
  ...
  },
  {
    ...
    howTo: [
      ...
      "QuickBooks Desktop receives the finalized Invoice only after Web Connector runs the staff-action writer step and QuickBooks accepts the invoice request. The invoice uses Peter's Custom S.O Invoice layout, keeps the QuickBooks Sales Order number, and prints the final customer, fulfillment, package, and instruction details.",
      "For a card order, the next QuickBooks writer step applies the Stripe payment specifically to that new Invoice. After QuickBooks accepts it, the Invoice should show the payment, a zero balance, and Paid status. A pay-by-invoice order deliberately skips this payment step and remains open in accounts receivable.",
    ...
    watch: [
      ...
      "QuickBooks Invoice posting depends on successful final Stripe charge, final weights or quantities, mapped QBD items, Peter's configured invoice template, and later Web Connector writer runs for the Invoice and its applied card payment.",
      ...
      "Stripe success does not by itself make the QuickBooks Invoice paid. For card orders, verify both the Invoice writer job and its invoice-specific Receive Payment job before expecting a zero QuickBooks balance.",
  ...
  },
  {
    ...
    watch: [
      ...
      "Once primary-contact confirmation is activated, changing the primary mobile retires the previous destination. Past addresses/orders stay historical; pending order texts to an old number are held rather than redirected. New order-text consent is collected separately at checkout.",
      "During the rollout, customers can choose Do this later and reach their account without claiming that their details were confirmed. The account keeps a reminder, and a new login can prompt again. Deferring does not change any contact, receipt address or text choice.",
      "If full confirmation is not yet available, the profile phone editor can use the existing account update. It saves the number and the customer's displayed marketing choice but does not claim confirmation. Existing orders keep their recorded contacts; ask the office to handle an existing-order change. A confirmed primary contact cannot be overwritten through this older path.",
  ...
  },
  {
    ...
    howTo: [
      ...
      "Staff-created accounts do not enroll customers in marketing texts. The customer must use the unchecked marketing-text checkbox themselves during storefront account signup or first-login contact verification.",
      ...
      "Choose fulfillment type, substitution preference, delivery notes, and order notes. Create Draft and Choose Date then shows the calendar when it has been activated, or the current scheduling fields while it is switched off. For regional pickup, choose a location in the customer's state first.",
      "Check Customer identity and order details verified after reading the order back to the customer.",
      "For Collect card by phone, check the customer authorization box and create the draft. Choose and confirm an available date and window before the card form appears. The date, staff access and inventory are checked again before card confirmation and order placement.",
      "For Send customer checkout link, create the draft and confirm its date. Only then is the link prepared and, if the email box is checked, emailed. A customer opening an expired date must confirm a current date in checkout. The customer may choose the optional order-text box themselves.",
    ],
    watch: [
      "Never enter card details without explicit customer authorization.",
      "Once the calendar is activated, an empty calendar or unavailable route is not permission to type a date manually. Choose another available option or ask the office to review the operating schedule. Changed draft details require a new draft and date confirmation.",
      "If the screen says card payment was confirmed but the order was not, open order support and reconcile the payment before taking another payment.",
      ...
      "If a line needs an override, the note should explain what the customer accepted or who approved it. After choosing the actual date, explicitly confirm that approval still applies to these quantities and that date. Changing the selected date clears that confirmation. A note alone does not approve changed details.",
  ...
  },
  {
    ...
    howTo: [
      ...
      "Use the queue filters to find orders by order number, customer email/name, ZIP, fulfillment method, or Ship / pickup date range. Date filters and sorting use the day the order leaves the plant or is collected there. Preparation and customer arrival/pickup dates are shown separately. An older shipped or regional order with no dispatch date stays visible without a date filter and needs office review; its customer arrival date is not a shipping date. Turn on Group by date to work one dispatch day at a time.",
    ...
    watch: [
      ...
      "Card orders cannot be fulfilled until the final Stripe charge succeeds. Approved pay-by-invoice orders keep their separate invoice-and-ship workflow.",
      "If fulfillment says payment status could not be verified, hold the order and retry fulfillment after the service recovers. Do not charge the card again or switch payment methods to clear this error. If it continues, ask a manager to reconcile the Medusa order and Stripe payment before retrying.",
  ...
  },
  {
    ...
    watch: [
      ...
      "The refund-reconciliation launch candidate keeps Stripe's refund reference and checks later outcomes without sending another refund. Pending, failed, canceled or action-required results need review. A refund made directly in Stripe can be recorded as a provider-only finding; it does not create a matching Medusa refund or QuickBooks credit automatically. Ask the accounting owner to reconcile those records before taking another money action.",
  ...
  },
  {
    ...
    useFor: [
      ...
      "Reviewing whether a customer explicitly opted into marketing texts through a customer-controlled storefront checkbox.",
      "Understanding the separate, order-scoped UPS shipping and tracking text choice shown at checkout.",
    ...
    howTo: [
      ...
      "For SMS marketing, confirm the profile has current marketing-only consent with the customer-controlled source, timestamp, copy version, phone number, provider, program, and exact disclosure before asking for a Twilio campaign send.",
      "For UPS shipping and tracking texts, the customer must choose the separate unchecked box during checkout. The box is not shown for pickup or Atlanta delivery. That choice applies only to the UPS-shipped order and allows up to six messages for the order.",
      "After an opted-in order is placed, the program attempts one required non-promotional enrollment confirmation. After that, only a real UPS shipment or tracking event can trigger an order text. Pickup-ready and local-delivery texts stay off; do not use Ready ship, a final charge, a delivery closure, or a staff promise as a substitute.",
      "If a customer asks about an order, verify fulfillment and tracking in Medusa and use phone or email support. Never promise that a text will arrive.",
    ...
    watch: [
      ...
      "The marketing toll-free program remains marketing-only. The dedicated order-update program is separate and limited to an enrollment confirmation plus real UPS shipping and tracking updates for an order with its own valid checkout consent.",
      "Order-text consent may be collected before the dedicated sender is approved. SMS sends only after carrier verification is approved and the backend gate is enabled; until then, email remains the operational channel.",
      "STOP opts the phone out of order updates only. START or UNSTOP may restore carrier delivery, but Medusa sends only while a still-valid consent exists for that order. HELP returns program and support information.",
  ...
  },
  {
    ...
    useFor: [
      ...
      "Maintaining plant-closure, UPS-pickup, and UPS-delivery blackout calendars.",
    ...
    howTo: [
      "Checkout reads fulfillment settings from Strapi. Carrier shipping also needs reviewed item weights and a published packing policy; missing inputs can make that shipping option unavailable.",
      "The new calendar starts switched off. Until the office approves activation, checkout keeps the existing scheduling controls and staff can enter the agreed date after creating a draft. UPS still needs verified transit dates and approved cold-chain packing. Switching the calendar on requires approved operating data; a calendar outage after activation does not reopen manual scheduling. In Strapi Checkout, the operating owner must approve and publish the Fulfillment Calendar Policy, with coverage dates, Eastern cutoffs, preparation times and pickup windows. Historical cutoffs are not an approval for a new calendar.",
      "Before calendar activation, the release owner must verify a successful preview order for plant pickup, Atlanta delivery, Southeast pickup and UPS shipping. A cart that already has a confirmed calendar date keeps its checks even during rollback. If that date cannot be verified, ask the release owner to restore the calendar; do not erase the date or type a replacement to get an order through.",
      "Atlanta delivery uses ZIP/day configuration and delivery windows.",
      "Southeast pickup uses confirmed Strapi route dates and Calendar Windows, Calendar Cutoff Days/Time and Calendar Preparation Days/Pack Time. An empty or past-only route has no bookable dates. Eligibility is state-wide: a Tennessee address can choose any active Tennessee stop, while a Texas address is shown Dallas Consolidated Shipping.",
      "UPS shipping uses shipping settings, origin ZIP, fallback amount, and the customer's requested arrival date. Ground is offered only where the transit table is 3 business days or less; longer-distance ZIPs use 3 Day Select as the baseline with 2nd Day and Overnight as paid premium options. The approved calendar controls UPS dispatch and arrival days. A separate approved transit fallback must name its origin, destination ZIP prefix, service, business days, revision and coverage. The carrier estimate is checked when a customer confirms a date; a changed estimate needs another explicit confirmation.",
      "In the Checkout singleton, maintain Fulfillment Blackout Dates by date and label. Mark each row separately for plant operations, UPS pickup, and UPS delivery; one date may block any combination of the three.",
      "Peter's SAM weight entries are retained for review. A value used to allow extra box space is not the food's physical weight. Engineering imports approved records by the stable QuickBooks item identity, keeping physical pounds or ounces, the number of units in a pack, and the separate box-space allowance distinct.",
      "In Strapi cold-chain settings, save a draft of the seasonal packing policy with its approval, revision and effective dates. Each ice rule names the UPS service, box size, maximum elapsed hours and blocks of dry ice per physical box. Small boxes can have shorter coverage or different block quantities. Record the delay allowance, permitted services and boxes, and maximum exposure and gross weight. The dates must cover packing through arrival plus the delay allowance; carrier business days stay separate. Publish only after Peter approves the values. If an earlier draft has an old ice multiplier, review the explicit block rules and clear that old field before publishing.",
      ...
      "Keep the three cost inputs current: dry-ice price per pound, physical pounds in one dry-ice block, and the cost of each box size. Minimum dry ice is a separate floor in pounds, not the block size. The rule's blocks per box times the block weight gives ice pounds, subject to that minimum. Fractional blocks require Peter's approval; the number of physical boxes is always whole. Each box needs measured dimensions and empty-box weight, a gross-weight limit, usable space and reviewed space per pound of ice. Larger blocks can require another box or make a service unavailable. Changing only ice price changes cost, not ice pounds. These packing costs are separate from what the customer pays for shipping.",
      "Once item weights and packing rules are approved, checkout can fall through from a compatible forecast to WWEX / Unishippers quotes, then Strapi shipping-zone rates. A historical forecast must be revalidated for the new physical-weight inputs before it can price these orders. A missing weight is a review problem, not a reason to accept a one-pound quote.",
      "At pack/finalize, UPS package rows are the source for final packed-box rating. Enter each box or cooler, its full measured packed weight, and its dimensions or an approved box type with configured dimensions. Open Accepted packing estimate in Pack & Finalize to review the policy, elapsed hours, delay allowance, estimated boxes and costs recorded with the order. Later catalog or dry-ice edits do not rewrite that estimate. Actual packing and the final customer shipping charge need their own review.",
    ],
    watch: [
      "Confirmed pickup and delivery windows display the published label and timezone. Calendar source changes are read again when a date is selected and when an order is submitted. Missing, expired or incomplete operating data stops date selection; it never activates a generic schedule.",
      "An edited basket, address, shipping service, expired quote or changed policy can require the shopper to refresh dates and confirm again. Never promise a date by typing it into cart metadata.",
      "The customer chooses an arrival or pickup date; the backend separately records the plant preparation and dispatch dates. QuickBooks Sales Order Due Date is the dispatch date. An institutional invoice payment deadline is separate and follows the QuickBooks account terms unless an explicit invoice deadline was approved. Check both documents; a dispatch date must not make an invoice payable early. Accepted orders keep their original promise; a later route cancellation needs staff exception handling and customer contact.",
      "Phone-order drafts check the calendar before payment. A cart with a confirmed calendar promise must keep validating that promise even if activation is later switched off; refresh its dates if validation fails. Release gate: the post-order shipping-override flow still needs the approved amendment workflow, including calendar, price and inventory consequences. Do not use its free-form date field to replace an accepted promise.",
      "Keep blackout meanings separate. A UPS no-pickup date blocks outbound dispatch, a UPS no-delivery date blocks customer arrival, and a plant-operations closure blocks pack-out plus local pickup/delivery routes. A UPS holiday does not automatically mean the plant is closed.",
      "After changing a route date or blackout, publish the Strapi record and verify checkout using an address in the affected state or ZIP.",
      "Keep cold-chain settings published: unpublish and delete are blocked, including for disabled settings. To withdraw carrier quotes, save Enabled = false as a draft and publish it. The release owner must first confirm the matching packing backend is active; older backend behavior can use defaults instead. Allow up to five minutes for the quote cache to refresh, then verify the published setting and affected service. This does not cancel accepted orders.",
      ...
      "If shipping asks for a weight or packing review, escalate the affected item and order to the shipping owner. Do not invent a weight or substitute the SAM space allowance for food weight. A reviewed import starts with one approved item and readback before a batch is permitted.",
      "UPS checkout needs a published, approved customer shipping policy as well as an approved packing policy. The price policy states whether fallback tables contain freight alone or already include packaging, gives a maximum customer shipping price, and records approval and effective dates. Missing, expired or inconsistent rules make UPS unavailable; pickup and local delivery follow their separate rules.",
      "A protected quote lasts 15 minutes. If the basket, address, packing or price policy changes, refresh the quote and select again. Free-shipping promotions still apply through Medusa. Customer shipping, its discount, estimated freight and box/ice costs are separate records.",
      "For orders accepted under the retain-accepted policy, a final carrier quote updates the cost record without replacing the customer's shipping charge. A missing quote or accepted price contract holds a new charge. Older orders without that contract need a reviewed migration or amendment; do not backfill a guessed price. Changed orders still need the approved amendment workflow.",
      "Release gate: Peter must approve the customer policy and worked examples, and the coordinated checkout, invoice, payment, receipt and QuickBooks rehearsal must pass before activation. This source candidate alone does not approve new prices.",
  ...
  },
  {
    ...
    howTo: [
      ...
      "Email remains the source for order confirmations, payments and final charges, cancellations, refunds, and fallback communication even when a customer chooses UPS shipping and tracking texts.",
  ...
  },
  {
    ...
    watch: [
      ...
      "Catalog and pickup-route backfills require a verified backup plus a write-capable admin session. Run the dry-run audit first and verify one published record through the public storefront path before continuing.",
  ...
  },
  {
    ...
    howTo: [
      ...
      "Synchronization Status measures the age of the last Web Connector sign-in. The displayed threshold comes from the sync monitor, and all status times are shown in UTC. An old successful sign-in or a cached open-session flag does not prove the connector is running now.",
      "If status becomes stale or unavailable, contact the monitoring owner and check the actual Web Connector and company file. An unavailable queue is unknown, not empty. The screen refreshes automatically and removes current counts after a failed read.",
      "A recovery means session freshness returned. Verify each queued order's transaction and accounting-action receipt separately before saying it posted. A recorded monitoring event still needs confirmation that the responsible operator received it.",
      ...
      "Customer checkout instructions stay in the QuickBooks Sales Order memo for planning and print as their own description row on the final Invoice.",
      "Final Invoices use Peter's Custom S.O Invoice template and QuickBooks' normal invoice-number sequence. They should show Bill To and Ship To, the original QuickBooks Sales Order number, specific delivery or UPS service, Subtotal, packed shipper containers, and any customer instruction. New orders must not include a CCC card-fee line.",
      "For a paid card order, QuickBooks should apply the received card payment to that exact Invoice and then show a zero balance. Pay-by-invoice orders intentionally remain unpaid in QuickBooks accounts receivable until Peter records Zelle, check, or wire payment.",
    ...
    watch: [
      ...
      "Catch-weight orders should post to QuickBooks as estimated sales orders when placed. Card orders post finalized invoices only after the final Stripe charge succeeds; approved pay-by-invoice orders post the finalized unpaid invoice when staff releases them.",
  ...
  },
  {
    ...
    howTo: [
      ...
      "Once refund reconciliation is activated, later Stripe status checks remain separate from the original refund record. Reporting counts one successful-refund record per Stripe refund; a later failure is a separate exception, not an automatic correction to accounting or GA4 revenue. Stripe's reported success also does not prove the credit is visible on the customer's bank statement. The coordinated release and provider rehearsal are still required.",
      ...
      "In the launch tracking candidate, an accepted order is counted once at its original checkout amount and date. A later final weighed charge is recorded separately. Customer Communications order totals after the cutover represent accepted order estimates, not money collected; older history is not automatically recalculated.",
      "If tracking is delayed, ask the analytics owner to check the saved delivery queue and its activation date. Each destination can recover separately. Do not place another order, collect another payment, or delete tracking receipts to make a report update. A delivery acknowledgment still needs to be checked against the reporting system.",
      "Launch rehearsal orders stay marked as tests and use separate test reports. Ask the analytics owner to confirm the test destinations before the rehearsal. Test purchases must not increase production sales, start marketing messages, or be relabeled as real orders; a test report does not prove production reporting is ready.",
      "The browser-tracking launch candidate checks the current cookie choice for every page, product and cart event. Declining analytics stops these events; accepting later does not replay earlier browsing. Test browsing uses verified separate destinations and does not start production marketing. The analytics owner must finish the backend producer checks and verify the tag manager consent setup before the live rehearsal.",
      "The confirmation page no longer supplies a separate purchase count in the launch candidate. Purchases come from the backend order record. Missing browser tracking must not be repaired by placing another order or marking a test as a real sale.",
      "The paired communications tracking candidate ignores test activity and declined analytics consent, and refuses browser claims of purchases or other backend-owned events. Missing event classification or tracking configuration means measurement is unavailable; it does not mean an order or payment failed. Ask the analytics owner to repair the sender without repeating the customer action. Remaining backend senders and older back-in-stock tracking must be checked before activation.",
      "The customer-tracking candidate saves the consent and test status supplied with the original account change. Later profile edits cannot rewrite that reporting record. Account details and required service email continue independently of analytics consent; a delayed report should be recovered from its saved event, never by repeating the customer's account change.",
      "The account-welcome candidate sends to the address saved when registration succeeded, independently of analytics-cookie permission. An address change before sending stops that welcome rather than redirecting it. A queued message or provider name is not a delivery receipt; the communications owner must verify the actual Postmark message and reconcile an uncertain attempt before retrying. Do not repeat signup to repair email. Test signups cannot start production welcomes, and the coordinated release and provider checks are still pending.",
      "The cart-recovery candidate keeps the original cart activity and recorded email permission. Declining analytics cookies does not cancel an existing email subscription, and accepting email later does not authorize recovery of earlier activity. New cart activity, checkout completion or revoked email permission stops an old recovery sequence. Suppressed emails are not counted as sent. Ask the communications owner to investigate saved activity before restarting a sequence or contacting the customer.",
      "The launch audience candidate holds calendar enrollment and campaigns using saved warehouse audiences when the latest refresh failed, the definition changed, or the result is more than a day old. Unavailable does not mean there are no customers in the audience. Ask the communications owner to restore a successful refresh before retrying; do not clear old membership or treat a fresh count as permission to contact customers. Consent, approved audience activation and provider checks still apply.",
      "The shipping-report candidate separates the price accepted at checkout from collected payment, estimated packing and carrier bills. An unavailable cost or margin is not zero. Inventory reports follow saved allocation changes; retries are not new allocations. Ask the analytics owner to recover the saved report before repeating a stock or money action. Actual packing, charges and bills still need reconciliation before a margin is treated as verified.",
    ...
    watch: [
      ...
      "Unavailable means the comparison could not be completed; it does not mean zero sales or a passed launch check. Keep acquisition and revenue experiments on hold until the missing source and the comparison are verified.",
      "Purchase events currently tied to a later final charge may disagree with a placement-estimate report. Do not treat that difference as a customer payment failure or change accounting records to make an analytics report match.",
...
const playbooks: Playbook[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "If the order is blocked, check customer/address data, missing QBD ListIDs, Peter's invoice-template configuration, the Subtotal item, native sales-tax item resolution for the destination ZIP, shipping item config, discount item config, or invalid line amounts.",
      ...
      "Before final charge, expect a QuickBooks Sales Order, not an Invoice. After Charge Card & Release, expect one writer run for the finalized Invoice and a following run for the payment applied to that Invoice.",
      "If Stripe charged successfully but QuickBooks has no final Invoice, check the QBD Invoice writer job and Web Connector staff-action step before assuming the card charge failed. If the Invoice exists but is not Paid, check the following Receive Payment writer job.",
    ],
  },
  {
  ...
  },
  {
    ...
    steps: [
      ...
      "Initial administrator access is tied to the approved account identity. Another super admin can change or revoke it in Team Access with a reason and confirmation. You cannot remove your own super admin access.",
      "Every staff access change requires the affected person to sign in again. Old sessions cannot regain staff powers by refreshing or after access is restored. The server checks current permissions for privileged actions.",
      "If every storefront administrator is locked out, the separately configured recovery administrator must use the audited staff-access action. Record the reason and verify the restored account with a fresh sign-in; do not edit role metadata directly.",
