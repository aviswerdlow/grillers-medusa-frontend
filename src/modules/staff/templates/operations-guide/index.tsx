...
const sections: GuideSection[] = [
  ...
  },
  {
    ...
    howTo: [
      ...
      "The account-welcome candidate keeps required production service email on by default, independently of analytics-cookie permission. An explicit delivery pause retains the original signup for recovery. It sends to the address saved when registration succeeded; an address change before sending stops that welcome rather than redirecting it. A queued message or provider name is not a delivery receipt. Ask the communications owner to verify the actual Postmark message and reconcile an uncertain attempt before retrying. Do not repeat signup to repair email. Test signups cannot start production welcomes, and the coordinated release and provider checks are still pending.",
      ...
      "The launch audience candidate holds calendar enrollment and campaigns using saved warehouse audiences when the latest refresh failed, the definition changed, or the result is more than a day old. Unavailable does not mean there are no customers in the audience. Failed refreshes and held calendar enrollment alert the communications owner. A separate refresh checks failed or missing audiences every 15 minutes and renews successful ones after six hours, without sending messages. Ask the owner to complete the refresh-only check before activation; do not use Run flows to repair membership because it can send due campaigns. Do not clear old membership or treat a fresh count as permission to contact customers. Consent, approved audience activation and provider checks still apply.",
